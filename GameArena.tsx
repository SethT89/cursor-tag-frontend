...
// Smoothed position tracker for each remote player
...
const GameArena: React.FC<GameArenaProps> = ({
  ...
  // Update targets when server sends new positions
  useEffect(() => {
    players.forEach(p => {
      if (p.id === localPlayerId) return; // local player uses raw position
  ...
  // Animation loop — interpolate toward targets at 60fps
  useEffect(() => {
    const LERP = 0.25; // 0 = no smoothing, 1 = instant. 0.25 feels good
    const loop = () => {
      let changed = false;
  ...
  // Get display position for a player
  const getPos = (p: Player) => {
    if (p.id === localPlayerId) return { x: p.x, y: p.y };
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        transform: 'translateX(-50%)', zIndex: 50, textAlign: 'center',
      }}>
        <div style={{
      ...
      {/* Render all cursors using smoothed positions */}
      {players.map(p => {
        const pos = getPos(p);
        return <PlayerCursor key={p.id} player={p} isLocal={p.id === localPlayerId} x={pos.x} y={pos.y} />;
      })}
...
const PlayerCursor: React.FC<{ player: Player; isLocal: boolean; x: number; y: number }> = ({ player, isLocal, x, y }) => {
  ...
  return (
    <div style={{
      ...
      // No CSS transition — smoothing is handled by the JS animation loop
    }}>
      {isIt && (
        <>
          <div style={{
